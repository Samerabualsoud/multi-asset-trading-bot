...
class MultiAssetTradingBot:
    ...
    def calculate_precise_entry(self, symbol, signal, rates):
        ...
        # ENHANCED STRATEGY: Multiple timeframe and indicator confirmation
        
        # Calculate EMA (faster response than SMA)
        ...
        # CRITICAL REQUIREMENT 1: Strong trend alignment
        if signal == 'BUY':
            # Enhanced: Check price above MA20, MA50, AND EMA20
            if current_price > ma20 and current_price > ma50 and current_price > ema20:
                ...
                if ma20 > ma50 and ema20 > ma20:
                    ...
                    reasons.append("Very strong uptrend (EMA>MA20>MA50)")
                elif ma20 > ma50:
                    confidence += 35  # Strong uptrend
                    reasons.append("Strong uptrend (MA20>MA50)")
                else:
                    confidence += 15  # Weak uptrend
                    reasons.append("Weak uptrend")
            else:
                # REJECT if not above MAs
                reject_reasons.append("NOT above MA20/MA50/EMA20")
            ...
            # CRITICAL REQUIREMENT 3: MACD confirmation (trend strength)
            if macd > 0:
                confidence += 10
                reasons.append("MACD bullish")
            
            # CRITICAL REQUIREMENT 4: RSI not overbought
            if rsi > 75:
            ...
            # Enhanced: Check price below MA20, MA50, AND EMA20
            if current_price < ma20 and current_price < ma50 and current_price < ema20:
                # Check if MAs are aligned (downtrend)
                if ma20 < ma50 and ema20 < ma20:
                    confidence += 50  # Very strong downtrend
                    reasons.append("Very strong downtrend (EMA<MA20<MA50)")
                elif ma20 < ma50:
                    confidence += 35  # Strong downtrend
                    reasons.append("Strong downtrend (MA20<MA50)")
                else:
                    confidence += 15  # Weak downtrend
                    reasons.append("Weak downtrend")
            else:
                # REJECT if not below MAs
                reject_reasons.append("NOT below MA20/MA50/EMA20")
                return entry_price, 0, "REJECTED: " + ", ".join(reject_reasons), atr
            ...
            # CRITICAL REQUIREMENT 4: RSI not oversold
            if rsi < 25:
    ...
    def execute_trade(self, opportunity):
        ...
        # NO STOP LOSS - User requested unlimited risk
        request = {
            ...
            "sl": 0.0,  # NO STOP LOSS
            "tp": tp,   # Keep take profit
            "deviation": 20,
            "magic": 234000,
            "comment": "No SL - TP Only",
